"""matplotlib-Renderer für Mini-Charts als PNG-Bytes (Qt-frei).

Wird von `io/html_report.py` (Base64-Embed) und `io/multi_report_exporter.py`
(Excel-Image) konsumiert. Liegt bewusst im `io`-Layer, weil weder die
Caller noch die Render-Logik selbst etwas mit der Qt-UI zu tun haben –
der Pixmap-Wrapper für die UI sitzt in `ui/widgets/chart_renderer.py` und
ruft seinerseits die Bytes-Funktionen hier auf.
