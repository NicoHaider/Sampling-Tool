...
def _make_dataset(n: int = 5) -> tuple[Dataset, tuple[DatasetRow, ...]]:
    """Sprint-11.1-Helper: liefert (Dataset, rows) – Dataset hält keine rows mehr."""
    rows = tuple(
        DatasetRow(
            row_id=i,
            values={
    ...
    return (
        Dataset(name="t", columns=("Name", "Betrag", "Datum"), row_count=len(rows)),
        rows,
    )
...
class TestDatasetTableModel:
    ...
    def test_format_value_handles_datetime_none_bool(self) -> None:
        ds = Dataset(name="x", columns=("a", "b", "c", "d"), row_count=1)
        rows = (
            DatasetRow(
                row_id=1,
                values={
    ...
    def test_datetime_with_midnight_shows_only_date(self) -> None:
        ds = Dataset(name="x", columns=("dt",), row_count=1)
